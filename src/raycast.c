...
static void		fill_ray(t_rc_utils *rc_utils, t_ray *ray, float ray_angle)
{
	if (rc_utils->vert_hit_distance < rc_utils->horz_hit_distance)
	{
		...
		ray->length_from_leftside = floor(ray->wall_hit_y / TILE_SIZE) *
			TILE_SIZE + TILE_SIZE - ray->wall_hit_y;
	...
	else
	{
		...
		ray->length_from_leftside = -(floor(ray->wall_hit_x / TILE_SIZE) *
			TILE_SIZE - ray->wall_hit_x);
